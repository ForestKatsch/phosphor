import { z } from "zod";
import { parsePath, type ParsePath, type PathSegment } from "./path";
...
export class Router<TPath extends PathSegment[] = []> {
  ...
  constructor(path: PathSegment[] = [], parent?: Router<any>) {
    this.path = path;
    this.trie = parent?.trie ?? new Trie();
  }

  route<Path extends string>(path: Path): Router<[...TPath, ...ParsePath<Path>]> {
  ...
  >(
    ...
    const methods = this.trie.getValue(this.path) ?? new Map<string, HandlerEntry>();
  ...
  private describeEntry(entry: TrieEntry<Map<string, HandlerEntry>>): IntrospectedRoute {
    const methods: IntrospectedMethod[] = [];
    entry.value.forEach((handler, method) => {
      const methodEntry: IntrospectedMethod = { method: method.toLowerCase() as MethodName };
      if (handler.body) {
...
export interface Router<TPath extends PathSegment[] = []>
  extends RouterMethods<TPath> { }
