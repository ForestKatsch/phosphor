...
export class Trie<THandler> {
  ...
  insert(path: PathSegment[], value: THandler) {
    ...
    for (const segment of path) {
      switch (segment.type) {
        ...
        case "variable": {
          const existing = node.params.find((edge) => edge.name === segment.value);
          if (existing) {
            node = existing.node;
            break;
          }
  ...
  getValue(path: PathSegment[]): THandler | undefined {
    ...
    for (const segment of path) {
      switch (segment.type) {
        ...
        case "variable": {
          const next = node.params.find((edge) => edge.name === segment.value)?.node;
          if (!next) {
            return undefined;
          }
          node = next;
  ...
  private matchFromNode(
    ...
    for (const param of node.params) {
      ...
      const match = this.matchFromNode(param.node, segments, index + 1, nextParams, originalPath);
      if (match) {
        return match;
      }
    }
