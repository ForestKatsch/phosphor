...
import { BadRequestError, HttpError, InternalServerError, NotFoundError, ValidationError } from "./error";
...
export class Server {
  ...
  listener(_req: IncomingMessage, res: ServerResponse) {
    ...
    const run = async () => {
      ...
      result = await match.handler.handler(request as any);

      if (match.handler.response) {
        const responseResult = match.handler.response.safeParse(result);
        if (!responseResult.success) {
          throw new InternalServerError("Unable to safely parse and validate response body");
    ...
    run().catch((error) => {
      if (!(error instanceof HttpError)) {
        console.error(`uncaught exception serving ${method} ${url.pathname}: ${error.message}`);
        console.trace(error);
